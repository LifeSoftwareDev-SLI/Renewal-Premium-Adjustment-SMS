...
FROM (
    ...
    FROM
        LPHS.PHNAME P
        JOIN LCLM.LEDGER L ON P.PNPOL = L.LLPOL
...
sms_category := 'SSMS';
sms_text := 'Dear ' || rec1.PNSTA || ' ' || rec1.PNINT || ' ' || rec1.PNSUR || ',' || CHR(10) || 'The Renewal Premium for your SLI Life Policy No. '|| rec1.LLPOL ||', due on ' || to_char(rec1.LLDUE, 'yyyy-mm') || ', has been adjusted using funds from your suspense account.';
